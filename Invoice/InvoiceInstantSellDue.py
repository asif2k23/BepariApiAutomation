...
def test_invoice_sell_due():

    invoice_id_to_sell_due = 1142

    response = requests.post(
        url = f'{base_url}accounts/invoice/instant-sell-due/{invoice_id_to_sell_due}',
        headers = head
    )
...
def test_delivered_invoice_sell_due():

    invoice_id_to_sell_due = 1042
